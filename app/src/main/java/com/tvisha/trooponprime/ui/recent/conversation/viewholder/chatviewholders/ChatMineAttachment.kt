...
import com.tvisha.trooponprime.databinding.ConAttacchmentMessageMineItemBinding
...
class ChatMineAttachment(var binding:ConAttacchmentMessageMineItemBinding,var context: Context):RecyclerView.ViewHolder(binding.root) {
    fun bindData(message: ConversationModel){
        ...
        binding.txtMsg.text = if (message.caption!=null) message.caption!! else ""
    ...
    private fun setReplyMessage(message: ConversationModel){
        ...
        Log.e("replyMessage---> ","attachment "+message.message+"   "+message.original_message)
    ...
    private fun setAttachment(message: ConversationModel){
        var attachmentType = Helper.getFileIconPath(message.attachment)
        if (attachmentType==ConstantValues.Gallery.GALLERY_IMAGE || attachmentType==ConstantValues.Gallery.GALLERY_VIDEO) {
            Glide.with(context).load(message.attachment)
                .placeholder(ContextCompat.getDrawable(context, R.drawable.attachment_pin_icon))
        ...
        binding.ivVideoButton.visibility = if (attachmentType==ConstantValues.Gallery.GALLERY_VIDEO) View.VISIBLE else View.GONE

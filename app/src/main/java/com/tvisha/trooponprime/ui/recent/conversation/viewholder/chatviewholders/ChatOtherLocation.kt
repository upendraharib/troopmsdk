...
import com.tvisha.trooponprime.databinding.ConLocationMessageOtherItemBinding
...
class ChatOtherLocation(var binding:ConLocationMessageOtherItemBinding, var context: Context):RecyclerView.ViewHolder(binding.root) {
    ...
    private fun setLocationMessage(message: ConversationModel) {
        var jsonObject  = Helper.stringToJsonObject(message.message)!!
        if (jsonObject!=null) {
            Log.e("location==> ", jsonObject.toString())
            binding.clLocation.tvLocationName.text = jsonObject.optString("location_name")
            binding.clLocation.tvLocationAddress.text = jsonObject.optString("location_address")
